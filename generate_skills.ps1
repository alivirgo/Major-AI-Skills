...
foreach ($app_name in $apps.Keys) {
    $app_data = $apps[$app_name]
    foreach ($ai in $ai_models) {
        
        $features_list = ($app_data['features'] | ForEach-Object { "- **$_**" }) -join "`n"
        $issues_list = ($app_data['issues'] | ForEach-Object { "- ⚠️ $_" }) -join "`n"
        $commands_list = $app_data['commands'] -join "`n"
        
        $keywords_array = $ai['seo_keywords'] | ForEach-Object { $_ -replace '\{app\}', $app_name }
        $keywords = ($keywords_array -join ", ") + ", $app_name, $($app_data['platform']) utilities, AI troubleshooting"
        
        $content = $template `
            -replace '\{app\}', ($app_name -replace "_", " ") `
            -replace '\{ai\}', $ai['name'] `
            -replace '\{platform\}', $app_data['platform'] `
            -replace '\{persona\}', $ai['persona'] `
            -replace '\{desc\}', $app_data['desc'] `
            -replace '\{features_list\}', $features_list `
            -replace '\{issues_list\}', $issues_list `
            -replace '\{troubleshooting\}', $app_data['troubleshooting'] `
            -replace '\{commands_list\}', $commands_list `
            -replace '\{keywords\}', $keywords

        $filename = "$($app_name.ToLower())_$($ai['name'].ToLower())_skill.md"
...
Write-Host "All skills generated successfully."
